...
function Confirmation() {
  ...
  const clinic = useMockStore((state) =>
    state.clinics.find((item) => item.id === application?.clinic_id),
  );
  if (!hydrated)
    return (
  ...
  if (!application)
    return (
      <main className="min-h-screen bg-surface grid place-items-center p-4">
        <Card className="w-full max-w-lg">
          <CardContent className="p-8 text-center">
            <h1 className="font-display text-2xl font-semibold">Confirmation unavailable</h1>
            <p className="text-sm text-muted-foreground mt-3">
              This application reference is invalid or no longer available.
            </p>
        ...
        <CardContent className="p-8 sm:p-10 text-center">
          ...
          <div className="mt-6 rounded-xl bg-surface border p-4 text-sm text-left space-y-2">
            ...
            {clinic && (
              <>
                <p>
                  <span className="text-muted-foreground">Clinic:</span> {clinic.name}
                </p>
                <p className="flex items-center gap-1.5">
                  <Clock className="size-4 text-muted-foreground" /> Estimated response within{" "}
                  {clinic.response_time_hours} hours
                </p>
              </>
            )}
          </div>
