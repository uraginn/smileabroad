...
export const Route = createFileRoute("/_public/clinics/$slug")({
  validateSearch: (search: Record<string, unknown>) => ({
    roadmap: typeof search.roadmap === "string" ? search.roadmap : undefined,
  }),
          ...
          <Card>
            <CardContent className="p-6">
              ...
              <div className="mt-6 grid sm:grid-cols-2 gap-3 text-sm">
                <Row
                  icon={Star}
                  label={`${clinic.google_rating} Google (${clinic.google_reviews})`}
                />
                <Row
                  icon={Star}
                  label={`${clinic.trustpilot_rating} Trustpilot (${clinic.trustpilot_reviews})`}
                />
                ...
                <Row
                  icon={Hotel}
                  label={clinic.hotel_included ? "Hotel included" : "Hotel available"}
                />
                <Row
                  icon={Car}
                  label={clinic.transfers_included ? "Transfers included" : "Transfers on request"}
                />
          ...
          <Card className="sticky top-20">
            <CardContent className="p-6">
              ...
              <Button asChild variant="outline" className="w-full mt-2">
                <Link to="/clinics">Compare clinics</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
